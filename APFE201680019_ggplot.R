...
sepal_plot1 <- ggplot(iris, aes(x= Sepal.Length, y= Sepal.Width,col=Species)) + geom_point()
sepal_plot1 
...
sepal_plot2 <- ggplot(iris, aes(x= Sepal.Length, y= Sepal.Width,col=Species, size = Petal.Length)) + geom_point()
sepal_plot2
...
ggplot(mtcars, aes(x= wt , y= mpg, col=factor(cyl))) +geom_text(aes(label=cyl))
...
ggplot(diamonds, aes(x= clarity, y = carat, col = price)) + geom_point(alpha = 0.5) + geom_jitter()
...
ggplot(diamonds, aes(x= carat, y = price, col = factor(table))) + geom_point(alpha= 0.5) + facet_wrap(~cut)
